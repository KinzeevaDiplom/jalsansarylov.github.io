...
$(document).ready(function () {
  ...
  $(".search").keyup(function () {
    ...
    if (val != "") {
      for (let i = 0; i < colPage; i++) {
        if (
          stepNamesU[i].search(val) == -1 &&
          stepNamesL[i].search(val) == -1
        ) {
  ...
  function createNav() {
    for (let i = 0; i < colPage; i++) {
      let nameStep =
        i + 1 + ". " + $(".content--text__item:eq(" + i + ") h2").text();

      $(".step__chapter").append(
        $('<a href = "#' + i + '" class = "step__link">' + nameStep + "</a>")
      );
  ...
  function markerStep() {
    ...
    for (let i = 0; i < colPage; i++) {
      if (
        $(window).scrollTop() + $(".pop-box").height() >=
        $(".content__title:eq(" + i + ")").offset().top
      ) {
